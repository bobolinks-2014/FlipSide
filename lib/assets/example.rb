...
demo_url= 'http://online.wsj.com/articles/russia-says-ukraine-nato-ambitions-threaten-peace-efforts-1409828434'
demo_html = '<html><head><title>Ruby Demo | alchemyapi</title></head><body><h1>Did you know that alchemyapi works on HTML?</h1><p>Well, you do now.</p></body></html>'


# kimono_response = RestClient.get 'https://www.kimonolabs.com/api/9lezh1lc?apikey=GsBpFLhrLVtXl8mIDnFj8zv8rRVrsKTn'
...
# getting a weird response wrt sentiment on an entity level. a lot of stuff is missing. but, it appears that the "targeted sentiment" on their website works
# NEED TO FIGURE OUT HOW TO GET TARGETED SENTIMENT IN THIS API
response = alchemyapi.combined('url', demo_url, { 'extract'=>'keyword,entity', 'sentiment'=>1 })

# we can pick out tags based on relevence
...
if response['status'] == 'OK'
	...
	if response.key?('keywords')
		...
		for keyword in response['keywords']
			...
			puts "\tsentiment: #{keyword["sentiment"]['score']}"
			puts ''
		end
	end
...
# puts ''
#
# response = alchemyapi.entities('text', 'http://online.wsj.com/articles/russia-says-ukraine-nato-ambitions-threaten-peace-efforts-1409828434', { 'sentiment'=>1 })
